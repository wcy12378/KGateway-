"""全链路异步可观测性 — LangFuse 链路追踪 + 本地度量聚合。"""
...
logger = logging.getLogger("kgateway.core.observability")
...
class GatewayObserver:
    ...
    async def end_trace(self, ctx: TraceContext) -> Dict[str, Any]:
        ...
        if self._langfuse_client is not None:
            try:
                await asyncio.to_thread(self._export_to_langfuse, ctx)
            except Exception as exc:
    ...
    def _export_to_langfuse(self, ctx: TraceContext) -> None:
        """同步导出到 LangFuse（在子线程中执行）。"""
        try:
            trace = self._langfuse_client.trace(
                id=ctx.trace_id,
                name="kgateway_request",
                metadata={
            ...
            # 为每个 span 创建 LangFuse generation
